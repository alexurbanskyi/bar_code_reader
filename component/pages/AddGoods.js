...
function AddGoods({navigation}) {
  ...
  const fotoToggleModal = () => {
    setFotoModalVisible(!isFotoModalVisible);
  };
  ...
  useEffect(() => {
    if (barData) {
      const checkProduct = productsList.find(item => item.barData === barData);
      if (checkProduct) {
        console.log('checkProduct YESSS!');
        setBarData(null);

        toast.show('Product was added to List', {
          type: 'danger',
          placement: 'bottom',
          duration: 2000,
  ...
  // add prod item to Full list
  async function addProductToAsyncStorage(element) {
    let productsList = [];
    try {
      await AsyncStorage.getItem('ProdList').then(list => {
        //console.log('get from astorege -->', list);
        if (list != null) {
          productsList = [...JSON.parse(list)];
          //  console.log('productsList ---->', productsList)
        }
      });
      productsList.push({...element});
      // console.log('productsList + element ---->', productsList)
      await AsyncStorage.setItem('ProdList', JSON.stringify(productsList));
    } catch (e) {
      console.log('Error --->', e);
    }
  } //end add item
  ...
  function onSubmit(data) {
    if (isValidData) {
      ...
      addProductToAsyncStorage(productData);
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.elementContainer}>
        ...
        />
        {errors.name && (
          <Text style={styles.errorText}>Enter Name Required</Text>
        )}
      </View>
      <View style={styles.elementContainer}>
        ...
        />
        {errors.price && (
          <Text style={styles.errorText}>Enter Price Required</Text>
        )}
      </View>
      <View>
        ...
        <Pressable
          style={
            isValidData
              ? {...styles.addButton, ...styles.validButton}
              : styles.addButton
          }
      ...
      <Modal style={styles.modal} isVisible={isFotoModalVisible}>
        <View style={styles.modalContainer}>
          <View style={styles.fotoCamera}>
            <FotoCameraComponent
              setFotoModalVisible={setFotoModalVisible}
              setFotoData={setFotoData}
            />
          </View>
          <View style={styles.hidenButton}>
